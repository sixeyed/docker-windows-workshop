...
& docker $config image build -t $env:dockerId/signup-e2e-tests -f part-5\e2e-tests\Dockerfile .

& docker $config container run --env-file .\part-5\app\db-credentials.env $env:dockerId/signup-e2e-tests
